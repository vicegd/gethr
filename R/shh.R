...
#'
#' \code{shh_generateSymKeyFromPassword} stores the symmetric key, and returns its ID.
#'
#' @family shh functions
#'
...
#'
#' \code{shh_hasSymKey} checks if the whisper node has a symmetric key given an ID.
#'
#' @family shh functions
#'
#' @param id String - ID of the symmetric key.
#'
#' @return Boolean - \code{true} if the symmetric key exists, otherwise \code{false}.
#'
#' @examples
#' \donttest{
...
#' @family shh functions
#'
#' @return Object - Diagnostic information about current minimum PoW requirement, message size limit in bytes, memory size of the messages and number of current messages.
#'
#' @examples
#' \donttest{
#' shh_info()
...
#'
#' \code{shh_markTrustedPeer} marks specific peer trusted, which will allow it to send historic (expired) messages.
#'
#' @family shh functions
#'
...
#'
#' \code{shh_newKeyPair} generates a new public and private key pair for message decryption and encryption.
#'
#' @family shh functions
#'
#' @return String - Key pair ID on sucess.
...
#'
#' \code{shh_newMessageFilter} create a new filter within the node. This filter can be used to poll for new messages that match the set of criteria.
#'
#' @family shh functions
#'
...
#' @param allowP2P Boolean - Indicates if this filter allows processing of direct peer-to-peer messages (which are not to be forwarded any further, because they might be expired).
#'
#' @return String - Filter identifier.
#'
#' @examples
#' \donttest{
#' shh_newMessageFilter(symKeyID = "8e975ab6e0427396d3afb748678a09b036ebe389c1c5a6d39560adabdfdb08ca")
#' shh_newMessageFilter(privateKeyID = "3794e3b08a2962b066e19869283974dc6b396cfc87cdcd69a2d269f5f1bab3cb", minPow = 0.3, allowP2P = TRUE)
#' }
#'
#' @export
shh_newMessageFilter <- function(symKeyID = NULL, privateKeyID = NULL, sig = NULL, minPow = NULL, topics = NULL, allowP2P = NULL) {
  params <- list()
  if (!is.null(symKeyID)) params = append(params, list(symKeyID = symKeyID))
  if (!is.null(privateKeyID)) params = append(params, list(privateKeyID = privateKeyID))
  if (!is.null(sig)) params = append(params, list(sig = sig))
  if (!is.null(minPow)) params = append(params, list(minPow))
  if (!is.null(topics)) params = append(params, list(topics = topics))
  if (!is.null(allowP2P)) params = append(params, list(allowP2P = allowP2P))
  get_post("shh_newMessageFilter", list(params))
}

#' Symmetric key creation and storage.
#'
#' \code{shh_newSymKey} generates a random symmetric key and stores it under an ID, which is then returned. Can be used encrypting and decrypting messages where the key is known to both parties.
#'
#' @family shh functions
#'
#' @return String - Symmetric key ID on sucess.
...
#' New whisper message.
#'
#' \code{shh_post} creates a whisper message and injects it into the network for distribution.
#'
#' @family shh functions
#'
...
#' @param powTime Integer - Maximal time in seconds to be spent on proof of work.
...
#'
#' @return Boolean - \code{true} if the message was send, otherwise \code{false}.
#'
#' @examples
#' \donttest{
#' shh_post(symKeyID = "7bc23b46b61e8223ef49241bd23b94921fb1e2dd8fd7bcb58df4159e1f4d3759", ttl = 7, topic = "0x07678231", payload = "0x68656c6c6f", powTime = 2, powTarget = 3)
#' shh_post(pubKey = "0x0425670405b102c0ce487cefae7aa2bfd7b474b76bc8433499bec777bb15d6d8a6b95e3001d16de259bf3170ec4cff38f00321eedc8a808a2f2e67bec6b254a1b1", ttl = 7, payload = "0x68656c6c6f", powTime = 2, powTarget = 3)
#' }
#'
#' @export
shh_post <- function(symKeyID = NULL, pubKey = NULL, sig = NULL, ttl, topic = NULL, payload, padding = NULL, powTime, powTarget, targetPeer = NULL) {
  ...
  if (!is.null(symKeyID)) params = append(params, list(symKeyID = symKeyID))
  if (!is.null(pubKey)) params = append(params, list(pubKey = pubKey))
  if (!is.null(sig)) params = append(params, list(sig = sig))
  if (!is.null(topic)) params = append(params, list(topic = topic))
  if (!is.null(padding)) params = append(params, list(padding = padding))
  if (!is.null(targetPeer)) params = append(params, list(targetPeer = targetPeer))
  get_post("shh_post", list(params))
}

#' Maximal message size allowed by this node.
#'
#' \code{shh_setMaxMessageSize} sets the maximal message size allowed by this node. Incoming and outgoing messages with a larger size will be rejected. Whisper message size can never exceed the limit imposed by the underlying P2P protocol (10 Mb).
#'
#' @family shh functions
#'
